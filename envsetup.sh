...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^legendrom_") ; then
        LEGENDROM_BUILD=$(echo -n $1 | sed -e 's/^legendrom_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $LEGENDROM_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        LEGENDROM_BUILD=
    fi
    export LEGENDROM_BUILD
...
function print_lunch_menu()
{
    ...
    if [ "z${LEGENDROM_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the LegendROM model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch legendrom_$target-$variant
        fi
    fi
    return $?
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $LEGENDROM_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.legendrom.device | grep -q "$LEGENDROM_BUILD");
    then
        ...
        echo "The connected device does not appear to be $LEGENDROM_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.legendrom.device | grep -q "$LEGENDROM_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
